'use strict';

const types = require('./types');
const Handler = require('./handler');
const {
  DATA_UNIT_TYPE,
  PARCEL_HEADER,
  STREAM_HEADER,
  CHUNK_HEADER,
  CHUNK_PAYLOAD,
  ACK,
} = require('./constants');

const HANDLER_BUFFER_SIZE = Math.max(
  ...[DATA_UNIT_TYPE, PARCEL_HEADER, STREAM_HEADER, CHUNK_HEADER, ACK].map(
    typeName => types[typeName].size
  )
);

class Connection {
  constructor(transport) {
    this.transport = transport;

    const buffer = Buffer.allocUnsafe(HANDLER_BUFFER_SIZE);
    const handler = new Handler(buffer);

    handler.on(DATA_UNIT_TYPE, dataUnitType => {
      handler.type = dataUnitType;
    });

    handler.on(PARCEL_HEADER, () => {
      handler.type = DATA_UNIT_TYPE;
    });

    handler.on(STREAM_HEADER, () => {
      handler.type = DATA_UNIT_TYPE;
    });

    handler.on(CHUNK_HEADER, chunk => {
      handler.type = CHUNK_PAYLOAD;
      handler.type.size = chunk.size;
      handler.buffer = getPayloadBuffer(chunk);

      handler.once(CHUNK_PAYLOAD, payload => {
        chunk.payload = payload;
        handler.buffer = buffer;
        handler.type = DATA_UNIT_TYPE;
      });
    });

    handler.on(ACK, () => {
      handler.type = DATA_UNIT_TYPE;
    });

    this.transport.on('data', handler.receiveData);
  }
}

module.exports = Connection;
